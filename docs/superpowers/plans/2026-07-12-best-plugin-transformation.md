...
**Description:** Create custom plugin tools that enable the orchestrator to dispatch subagents in the background. The plugin exposes: `dispatch_background(agent, task, prompt)` → dispatch ID (returns immediately), `list_dispatches(status?)` → status of all dispatches, and `get_dispatch(dispatch_id)` → result. Uses the SDK's `session.create()` + `session.prompt()` under the hood.
...
- [ ] `dispatch_background` custom tool: takes `agent` + `task` + `prompt`, creates session, dispatches, returns dispatch ID
- [ ] `list_dispatches` custom tool: takes optional `status` filter, returns dispatch status list
- [ ] `get_dispatch` custom tool: takes dispatch ID, returns result/output
...
- Create: `docs/adr/ADRF-0001-use-adrs.md`
- Create: `docs/adr/ADRF-0002-agent-architecture.md`

**Description:** Set up an Architectural Decision Record (ADR) system following the MADR (Markdown ADR) convention. Document the two foundational architecture decisions: ADRF-0001 explains why ADRs were adopted, ADRF-0002 documents the agent architecture and depth-1 delegation mandate.
