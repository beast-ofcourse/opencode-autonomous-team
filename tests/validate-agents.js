    ...
    if (bash && typeof bash === 'object') {
      for (const [pattern, value] of Object.entries(bash)) {
        ...
        if (isAllowed) {
          ...
          const isDestructive = DESTRUCTIVE_PATTERNS.some(dp => patternLower.includes(dp));
          if (isDestructive) {
            report(
              filePath,
...
const validAgents = agents.filter(a => a.parsed && typeof a.parsed === 'object');
...
if (orchestrator && orchestrator.parsed) {
  const taskPerms = orchestrator.parsed.permission && orchestrator.parsed.permission.task;
